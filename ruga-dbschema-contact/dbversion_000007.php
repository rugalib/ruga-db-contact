...
CREATE TABLE `{$electronicAddressTable}` (
  ...
  PRIMARY KEY (`id`),
  INDEX `fk_{$electronicAddressTable}_{$contactMechanismTable}1_idx` (`{$contactMechanismTable}_id` ASC),
  INDEX `fk_{$electronicAddressTable}_changedBy_idx` (`changedBy` ASC),
  INDEX `fk_{$electronicAddressTable}_createdBy_idx` (`createdBy` ASC),
  CONSTRAINT `fk_{$electronicAddressTable}_{$contactMechanismTable}1` FOREIGN KEY (`{$contactMechanismTable}_id`) REFERENCES `{$contactMechanismTable}` (`id`) ON DELETE RESTRICT ON UPDATE RESTRICT,
  CONSTRAINT `fk_{$electronicAddressTable}_changedBy` FOREIGN KEY (`changedBy`) REFERENCES `User` (`id`) ON DELETE RESTRICT ON UPDATE RESTRICT,
  CONSTRAINT `fk_{$electronicAddressTable}_createdBy` FOREIGN KEY (`createdBy`) REFERENCES `User` (`id`) ON DELETE RESTRICT ON UPDATE RESTRICT
)
ENGINE = InnoDB
;
